import { cssParser, cssParserComponents, cssParserPalette, cssParserModern } from './parser.js';
...
export const cssMarkerModern = (breakpoints: { [key: string]: number }): string => {
	const css = cssParserModern();
	const startMarkerLength = markerBreakpoint.start.length;
	const endMarkerLength = markerBreakpoint.end.length;
